...
class MULTI_TOGGLE_PT_mesh_rules_panel(Panel):
    ...
    def draw(self, context):
        ...
        if not obj:
            layout.label(text="메쉬를 선택하세요.")
            return
            
        # 2. 상단 상태 표시
        box = layout.box()
        box.label(text=f"타겟 메쉬: {obj.name}", icon='MESH_DATA')
        
        # 가시성 표시 인디케이터 (규칙 실시간 평가)
        from ..core.live_preview import compile_rules, evaluate_advanced_expression
        
        rules = obj.multi_toggle_rules
        if len(rules.conditions) == 0:
            box.label(text="보임 (조건 없음)", icon='HIDE_OFF')
        else:
            expr = compile_rules(rules, scene.multi_toggle_globals)
            is_visible = evaluate_advanced_expression(expr, scene.multi_toggle_globals)
            
            if not is_visible:
                box.label(text="숨겨짐 (조건 불일치)", icon='HIDE_ON')
            else:
                box.label(text="보임 (조건 충족)", icon='HIDE_OFF')
            
        layout.separator()
        
        row = layout.row(align=True)
        row.operator("multi_toggle.add_condition", text="조건", icon='ADD')
        row.operator("multi_toggle.add_paren", text="괄호 추가", icon='PASTEDOWN')
